...
const PlaceRequest = () => {
  ...
  const goToMatch = () => {
    if (!form.title.trim()) {
      toast({ title: 'Укажите, что вы ищете', variant: 'destructive' });
      return;
    }
    if (!form.category) {
    ...
    if (!form.description.trim()) {
      toast({ title: 'Опишите вашу заявку', variant: 'destructive' });
      return;
    }
    if (!form.contact_name.trim() || !form.contact_phone.trim()) {
      toast({ title: 'Укажите контактное имя и телефон', variant: 'destructive' });
      return;
    }
    setStep(2);
    window.scrollTo(0, 0);
      ...
      <PageLayout>
        <div className="container mx-auto px-4 py-8 max-w-4xl">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Разместить заявку</h1>
            <p className="text-gray-600">
              Опишите, что вам нужно — и заявка автоматически улетит всем поставщикам,
              которые торгуют этим или похожим товаром. Не нужно искать каждого вручную.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 mb-8">
            {[
              ...
              <div key={s.n} className="flex items-center gap-2">
                ...
                <span className={`text-sm ${step >= s.n ? 'text-gray-900 font-medium' : 'text-gray-400'}`}>
                  {s.label}
                </span>
          ...
          {step === 1 && (
            <div className="space-y-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Input
                      placeholder="Например: Труба стальная профильная 40х40"
                      value={form.title}
                  ...
                  <div>
                    ...
                    <Textarea
                      placeholder="Опишите нестандартные требования: марка, размеры, ГОСТ, условия поставки, всё что важно"
                      rows={4}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Select value={form.unit} onValueChange={(v) => set('unit', v)}>
                        ...
                        <SelectContent>
                          {['шт', 'кг', 'т', 'м', 'м²', 'м³', 'л', 'упак', 'компл'].map((u) => (
                  ...
                  <div>
                    ...
                    <Input
                      placeholder="Например: до 15 сентября / срочно"
                      value={form.deadline}
              ...
              <div className="flex items-center justify-between bg-white border rounded-lg p-4">
                ...
                <Button onClick={goToMatch} size="lg">
                  Далее
                ...
                <CardContent>
                  {matched.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-sm text-gray-500">Заявку всё равно можно отправить — мы сохраним её и уведомим площадку.</p>
                    </div>
                  ) : (
                ...
                <Button onClick={handleSubmit} size="lg" disabled={isSubmitting}>
                  {isSubmitting ? (
                    ...
                    <>
                      ...
                      Отправить заявку {matched.length > 0 ? `(${matched.length})` : ''}
                    </>
                  )}
                </Button>
              </div>
            ...
            <Card className="text-center py-12">
              <CardContent>
                ...
                <p className="text-gray-600 max-w-md mx-auto mb-6">
                  {sentCount > 0 ? (
                    <>Ваша заявка «{form.title}» ушла <b>{sentCount}</b> поставщикам. Они свяжутся с вами напрямую по указанным контактам.</>
                  ) : (
                    <>Ваша заявка «{form.title}» сохранена. Как только появятся подходящие поставщики — она будет им отправлена.</>
                  )}
                </p>
                <Button onClick={resetForm} size="lg">
                  <Icon name="Plus" size={18} className="mr-2" />
                  Разместить ещё одну заявку
                </Button>
              </CardContent>
            </Card>
          )}
