import { Product } from "@/components/catalog/ProductCard";

export const productsData: Product[] = [
  {
    id: 1,
    name: "Труба стальная бесшовная 108x4 мм ГОСТ 8732-78",
    image: "/img/764d08a6-7946-4b1d-9c27-48e192211cc0.jpg",
    category: "Металлопрокат",
    seller: "ООО «МетПром-Сталь»",
    verified: true,
    price: 45600,
    oldPrice: 48000,
    unit: "за тонну",
    minOrder: "5 тонн",
    available: "120 тонн",
    rating: 4.9,
    reviews: 127,
    inStock: true,
    discount: 5,
    fastDelivery: true,
    description: "Высококачественная стальная труба для промышленного использования",
    detailedDescription: "Труба стальная бесшовная горячедеформированная диаметром 108 мм с толщиной стенки 4 мм. Изготовлена в соответствии с ГОСТ 8732-78 из углеродистой стали марки Ст20. Предназначена для строительства трубопроводов, работающих под давлением до 10 МПа, а также для изготовления металлоконструкций и машиностроения.\\n\\nОсновные характеристики:\\n• Диаметр наружный: 108 мм\\n• Толщина стенки: 4 мм\\n• Материал: углеродистая сталь Ст20\\n• Длина: 4-12 метров (мерная)\\n• Предел прочности: не менее 410 МПа\\n• Относительное удлинение: не менее 21%\\n\\nОбладает высокой прочностью, коррозионной стойкостью и долговечностью. Поставляется с сертификатами качества и соответствия ГОСТ. Возможна доставка автотранспортом или железнодорожным транспортом по всей России."
  },
  {
    id: 2,
    name: "Цемент ПЦ 400-Д20 навалом, М-400",
    image: "/img/d166a943-2618-4918-b162-2f653f5ae829.jpg",
    category: "Стройматериалы",
    seller: "АО «СтройБаза Регион»",
    verified: true,
    price: 3850,
    unit: "за тонну",
    minOrder: "20 тонн",
    available: "500+ тонн",
    rating: 4.8,
    reviews: 89,
    inStock: true,
    description: "Портландцемент высокого качества для строительных работ"
  },
  {
    id: 3,
    name: "Arduino Uno R3 + набор датчиков (комплект)",
    image: "/img/eb347072-5079-42a8-9320-9ff8ccc544f5.jpg",
    category: "Электроника",
    seller: "ТД «Электро-Компонент»",
    verified: true,
    price: 1290,
    oldPrice: 1450,
    unit: "за комплект",
    minOrder: "50 комплектов",
    available: "2,000+ шт",
    rating: 4.6,
    reviews: 234,
    inStock: true,
    discount: 11,
    fastDelivery: true,
    description: "Полный стартовый набор для изучения Arduino с документацией"
  },
  {
    id: 4,
    name: "Профнастил оцинкованный С8-1150-0.5",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Кровельные материалы",
    seller: "ООО «КровляСтрой»",
    verified: true,
    price: 485,
    unit: "за м²",
    minOrder: "200 м²",
    available: "5,000+ м²",
    rating: 4.7,
    reviews: 156,
    inStock: true,
    description: "Долговечный профнастил с полимерным покрытием"
  },
  {
    id: 5,
    name: "Упаковка картонная гофрированная 380x285x100",
    image: "/img/43a9e543-590b-44fb-b286-827f5d9b1ef7.jpg",
    category: "Упаковочные материалы",
    seller: "АО «УпакСервис»",
    verified: true,
    price: 28,
    unit: "за штуку",
    minOrder: "1,000 штук",
    available: "50,000+ штук",
    rating: 4.4,
    reviews: 203,
    inStock: true,
    description: "Прочная гофрированная упаковка для транспортировки"
  },
  {
    id: 6,
    name: "Кабель ВВГнг-LS 3x2.5 мм² (бухта 100м)",
    image: "/img/eb347072-5079-42a8-9320-9ff8ccc544f5.jpg",
    category: "Электротехника",
    seller: "ООО «ЭлектроКабель»",
    verified: true,
    price: 5670,
    unit: "за бухту",
    minOrder: "10 бухт",
    available: "300+ бухт",
    rating: 4.8,
    reviews: 94,
    inStock: true,
    description: "Негорючий кабель с низким дымо- и газовыделением"
  },
  {
    id: 7,
    name: "Бумага офисная А4 80г/м² белая (пачка 500л)",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Канцелярские товары",
    seller: "ТД «ОфисОпт»",
    verified: true,
    price: 245,
    oldPrice: 280,
    unit: "за пачку",
    minOrder: "100 пачек",
    available: "5,000+ пачек",
    rating: 4.2,
    reviews: 187,
    inStock: true,
    discount: 12,
    fastDelivery: true,
    description: "Высококачественная офисная бумага для печати"
  },
  {
    id: 8,
    name: "Комплект крепежа М12x60 с шайбами и гайками",
    image: "/img/764d08a6-7946-4b1d-9c27-48e192211cc0.jpg",
    category: "Крепёжные изделия",
    seller: "АО «КрепСталь»",
    verified: true,
    price: 89,
    unit: "за комплект (50 шт)",
    minOrder: "100 комплектов",
    available: "2,000+ комплектов",
    rating: 4.6,
    reviews: 142,
    inStock: false,
    description: "Оцинкованный крепеж повышенной прочности"
  },
  {
    id: 9,
    name: "Светодиодная лента RGB 5050 60LED/m (катушка 5м)",
    image: "/img/43a9e543-590b-44fb-b286-827f5d9b1ef7.jpg",
    category: "Светотехника",
    seller: "ООО «СветТех»",
    verified: true,
    price: 890,
    oldPrice: 1020,
    unit: "за катушку",
    minOrder: "50 катушек",
    available: "1,000+ катушек",
    rating: 4.3,
    reviews: 98,
    inStock: true,
    discount: 13,
    description: "Яркая светодиодная лента с возможностью смены цвета"
  },
  {
    id: 10,
    name: "Плитка керамическая глазурованная 20x20см",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Отделочные материалы",
    seller: "АО «КерамПром»",
    verified: true,
    price: 195,
    unit: "за м²",
    minOrder: "100 м²",
    available: "1,500+ м²",
    rating: 4.5,
    reviews: 89,
    inStock: true,
    description: "Износостойкая керамическая плитка для стен и пола"
  },
  {
    id: 11,
    name: "Пленка полиэтиленовая техническая 200мкм (рулон 3x100м)",
    image: "/img/d166a943-2618-4918-b162-2f653f5ae829.jpg",
    category: "Полимерные материалы",
    seller: "ООО «ПолиПак»",
    verified: true,
    price: 4280,
    unit: "за рулон",
    minOrder: "20 рулонов",
    available: "200+ рулонов",
    rating: 4.1,
    reviews: 67,
    inStock: true,
    description: "Прочная полиэтиленовая пленка для технических нужд"
  },
  {
    id: 12,
    name: "Штангенциркуль электронный 0-150мм с LCD",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Измерительные приборы",
    seller: "ТД «ТочПрибор»",
    verified: true,
    price: 2350,
    oldPrice: 2650,
    unit: "за штуку",
    minOrder: "10 штук",
    available: "80+ штук",
    rating: 4.7,
    reviews: 124,
    inStock: true,
    discount: 11,
    description: "Высокоточный электронный штангенциркуль с цифровым дисплеем"
  },
  {
    id: 13,
    name: "Смеситель для ванны с душем, хром",
    image: "/img/eb347072-5079-42a8-9320-9ff8ccc544f5.jpg",
    category: "Сантехника",
    seller: "ООО «СанТехОпт»",
    verified: true,
    price: 3450,
    unit: "за штуку",
    minOrder: "10 штук",
    available: "150+ штук",
    rating: 4.5,
    reviews: 89,
    inStock: true,
    description: "Качественный смеситель с керамическим картриджем"
  },
  {
    id: 14,
    name: "Унитаз компакт с бачком, белый",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Сантехника",
    seller: "АО «КерамСан»",
    verified: true,
    price: 8500,
    oldPrice: 9200,
    unit: "за комплект",
    minOrder: "5 комплектов",
    available: "60+ комплектов",
    rating: 4.6,
    reviews: 112,
    inStock: true,
    discount: 8,
    description: "Унитаз из сантехнического фарфора с микролифтом"
  },
  {
    id: 15,
    name: "Радиатор алюминиевый 500мм 10 секций",
    image: "/img/764d08a6-7946-4b1d-9c27-48e192211cc0.jpg",
    category: "Отопление и вентиляция",
    seller: "ООО «ТеплоТех»",
    verified: true,
    price: 4200,
    unit: "за радиатор",
    minOrder: "20 радиаторов",
    available: "300+ радиаторов",
    rating: 4.7,
    reviews: 156,
    inStock: true,
    description: "Эффективный алюминиевый радиатор с высокой теплоотдачей"
  },
  {
    id: 16,
    name: "Вентилятор канальный 100мм с таймером",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Отопление и вентиляция",
    seller: "ТД «ВентСистемы»",
    verified: true,
    price: 2850,
    unit: "за штуку",
    minOrder: "15 штук",
    available: "200+ штук",
    rating: 4.4,
    reviews: 78,
    inStock: false,
    description: "Бесшумный вентилятор с функцией отложенного выключения"
  },
  {
    id: 17,
    name: "Тормозные колодки передние (комплект)",
    image: "/img/43a9e543-590b-44fb-b286-827f5d9b1ef7.jpg",
    category: "Автокомплектующие",
    seller: "АО «АвтоДеталь»",
    verified: true,
    price: 1850,
    unit: "за комплект",
    minOrder: "50 комплектов",
    available: "500+ комплектов",
    rating: 4.6,
    reviews: 234,
    inStock: true,
    description: "Качественные тормозные колодки для легковых автомобилей"
  },
  {
    id: 18,
    name: "Масло моторное 5W-30 синтетика 4л",
    image: "/img/d166a943-2618-4918-b162-2f653f5ae829.jpg",
    category: "Автокомплектующие",
    seller: "ООО «МоторОйл»",
    verified: true,
    price: 2450,
    oldPrice: 2750,
    unit: "за канистру",
    minOrder: "30 канистр",
    available: "800+ канистр",
    rating: 4.8,
    reviews: 345,
    inStock: true,
    discount: 11,
    description: "Высококачественное синтетическое моторное масло"
  },
  {
    id: 19,
    name: "Стол офисный эргономичный 140x70см",
    image: "/img/eb347072-5079-42a8-9320-9ff8ccc544f5.jpg",
    category: "Мебель и интерьер",
    seller: "ООО «ОфисМебель»",
    verified: true,
    price: 12500,
    unit: "за штуку",
    minOrder: "10 штук",
    available: "100+ штук",
    rating: 4.5,
    reviews: 89,
    inStock: true,
    description: "Современный офисный стол с регулировкой высоты"
  },
  {
    id: 20,
    name: "Кресло офисное с подлокотниками",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Мебель и интерьер",
    seller: "АО «КомфортСит»",
    verified: true,
    price: 8900,
    oldPrice: 9800,
    unit: "за штуку",
    minOrder: "5 штук",
    available: "80+ штук",
    rating: 4.7,
    reviews: 124,
    inStock: true,
    discount: 9,
    description: "Эргономичное офисное кресло с поясничной поддержкой"
  },
  {
    id: 21,
    name: "Костюм рабочий летний (куртка+брюки)",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Спецодежда и СИЗ",
    seller: "ООО «РабОдежда»",
    verified: true,
    price: 1850,
    unit: "за комплект",
    minOrder: "50 комплектов",
    available: "500+ комплектов",
    rating: 4.3,
    reviews: 167,
    inStock: true,
    description: "Прочный рабочий костюм из хлопчатобумажной ткани"
  },
  {
    id: 22,
    name: "Каска защитная строительная с подбородочным ремнем",
    image: "/img/764d08a6-7946-4b1d-9c27-48e192211cc0.jpg",
    category: "Спецодежда и СИЗ",
    seller: "АО «БезОпасность»",
    verified: true,
    price: 450,
    unit: "за штуку",
    minOrder: "100 штук",
    available: "1,000+ штук",
    rating: 4.6,
    reviews: 87,
    inStock: true,
    description: "Ударопрочная каска с регулируемым размером"
  },
  {
    id: 23,
    name: "Мотокоса бензиновая 2.5 кВт с набором ножей",
    image: "/img/764d08a6-7946-4b1d-9c27-48e192211cc0.jpg",
    category: "Садовая техника",
    seller: "АО «СадТехника»",
    verified: true,
    price: 18500,
    oldPrice: 20000,
    unit: "за штуку",
    minOrder: "5 штук",
    available: "60+ штук",
    rating: 4.7,
    reviews: 94,
    inStock: true,
    discount: 8,
    description: "Профессиональная мотокоса для больших участков"
  },
  {
    id: 24,
    name: "Газонокосилка электрическая 1800Вт",
    image: "/img/d166a943-2618-4918-b162-2f653f5ae829.jpg",
    category: "Садовая техника",
    seller: "ТД «ГарденТех»",
    verified: true,
    price: 12800,
    unit: "за штуку",
    minOrder: "3 штуки",
    available: "40+ штук",
    rating: 4.5,
    reviews: 67,
    inStock: true,
    description: "Легкая и мощная газонокосилка с мульчированием"
  },
  {
    id: 25,
    name: "Средство для мытья посуды концентрат 5л",
    image: "/img/eb347072-5079-42a8-9320-9ff8ccc544f5.jpg",
    category: "Химия и моющие средства",
    seller: "ООО «ХимПродукт»",
    verified: true,
    price: 850,
    unit: "за канистру",
    minOrder: "30 канистр",
    available: "500+ канистр",
    rating: 4.4,
    reviews: 123,
    inStock: true,
    description: "Экономичное моющее средство с антибактериальным эффектом"
  },
  {
    id: 26,
    name: "Порошок стиральный автомат 15кг",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Химия и моющие средства",
    seller: "АО «БытХим»",
    verified: true,
    price: 2350,
    unit: "за мешок",
    minOrder: "20 мешков",
    available: "300+ мешков",
    rating: 4.2,
    reviews: 78,
    inStock: false,
    description: "Универсальный стиральный порошок для всех типов тканей"
  },
  {
    id: 27,
    name: "Мука пшеничная высшего сорта 25кг",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Пищевая продукция",
    seller: "ООО «АгроПродукт»",
    verified: true,
    price: 890,
    unit: "за мешок",
    minOrder: "50 мешков",
    available: "1,000+ мешков",
    rating: 4.6,
    reviews: 167,
    inStock: true,
    description: "Высококачественная пшеничная мука для хлебопекарен"
  },
  {
    id: 28,
    name: "Сахар-песок белый кристаллический 50кг",
    image: "/img/764d08a6-7946-4b1d-9c27-48e192211cc0.jpg",
    category: "Пищевая продукция",
    seller: "АО «СладПром»",
    verified: true,
    price: 2450,
    oldPrice: 2650,
    unit: "за мешок",
    minOrder: "40 мешков",
    available: "800+ мешков",
    rating: 4.5,
    reviews: 134,
    inStock: true,
    discount: 8,
    description: "Качественный белый сахар-песок для пищевой промышленности"
  },
  {
    id: 29,
    name: "Ткань хлопчатобумажная плательная 150см",
    image: "/img/43a9e543-590b-44fb-b286-827f5d9b1ef7.jpg",
    category: "Текстиль и ткани",
    seller: "ООО «ТекстильОпт»",
    verified: true,
    price: 340,
    unit: "за погонный метр",
    minOrder: "500 метров",
    available: "5,000+ метров",
    rating: 4.3,
    reviews: 89,
    inStock: true,
    description: "Натуральная хлопчатобумажная ткань высокого качества"
  },
  {
    id: 30,
    name: "Нитки швейные полиэстер 40/2 (катушка 5000м)",
    image: "/img/d166a943-2618-4918-b162-2f653f5ae829.jpg",
    category: "Текстиль и ткани",
    seller: "АО «НиткаПром»",
    verified: true,
    price: 285,
    oldPrice: 320,
    unit: "за катушку",
    minOrder: "200 катушек",
    available: "2,000+ катушек",
    rating: 4.4,
    reviews: 156,
    inStock: true,
    discount: 11,
    description: "Прочные полиэстеровые нитки для промышленного шитья"
  },
  {
    id: 31,
    name: "Тонометр автоматический с манжетой",
    image: "/img/eb347072-5079-42a8-9320-9ff8ccc544f5.jpg",
    category: "Медицинское оборудование",
    seller: "ООО «МедТехника»",
    verified: true,
    price: 2850,
    unit: "за штуку",
    minOrder: "20 штук",
    available: "100+ штук",
    rating: 4.6,
    reviews: 145,
    inStock: true,
    description: "Точный автоматический тонометр с памятью измерений"
  },
  {
    id: 32,
    name: "Стетоскоп медицинский двухголовочный",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Медицинское оборудование",
    seller: "АО «МедПрибор»",
    verified: true,
    price: 1450,
    oldPrice: 1650,
    unit: "за штуку",
    minOrder: "50 штук",
    available: "300+ штук",
    rating: 4.7,
    reviews: 98,
    inStock: true,
    discount: 12,
    description: "Профессиональный стетоскоп с алюминиевой грудной частью"
  },
  {
    id: 33,
    name: "Мяч футбольный профессиональный размер 5",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Спортивные товары",
    seller: "ООО «СпортОпт»",
    verified: true,
    price: 1850,
    unit: "за штуку",
    minOrder: "30 штук",
    available: "500+ штук",
    rating: 4.5,
    reviews: 187,
    inStock: true,
    description: "Качественный футбольный мяч для профессиональных игр"
  },
  {
    id: 34,
    name: "Гантели разборные 2x20кг с грифом",
    image: "/img/764d08a6-7946-4b1d-9c27-48e192211cc0.jpg",
    category: "Спортивные товары",
    seller: "АО «ФитнесОборудование»",
    verified: true,
    price: 4500,
    oldPrice: 5000,
    unit: "за комплект",
    minOrder: "10 комплектов",
    available: "80+ комплектов",
    rating: 4.6,
    reviews: 124,
    inStock: true,
    discount: 10,
    description: "Профессиональные разборные гантели с чугунными дисками"
  },
  {
    id: 35,
    name: "Холодильник двухкамерный No Frost 350л",
    image: "/img/eb347072-5079-42a8-9320-9ff8ccc544f5.jpg",
    category: "Бытовая техника",
    seller: "ООО «БытТехОпт»",
    verified: true,
    price: 48000,
    oldPrice: 52000,
    unit: "за штуку",
    minOrder: "3 штуки",
    available: "25+ штук",
    rating: 4.7,
    reviews: 89,
    inStock: true,
    discount: 8,
    description: "Энергоэффективный холодильник с системой No Frost"
  },
  {
    id: 36,
    name: "Микроволновая печь 25л с грилем",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Бытовая техника",
    seller: "АО «ТехноОпт»",
    verified: true,
    price: 8500,
    unit: "за штуку",
    minOrder: "10 штук",
    available: "60+ штук",
    rating: 4.4,
    reviews: 156,
    inStock: false,
    description: "Многофункциональная микроволновая печь с грилем и конвекцией"
  },
  {
    id: 37,
    name: "Конструктор детский пластиковый 500 деталей",
    image: "/img/43a9e543-590b-44fb-b286-827f5d9b1ef7.jpg",
    category: "Игрушки и товары для детей",
    seller: "ООО «ИгрушкиОпт»",
    verified: true,
    price: 2350,
    unit: "за набор",
    minOrder: "50 наборов",
    available: "300+ наборов",
    rating: 4.6,
    reviews: 245,
    inStock: true,
    description: "Развивающий конструктор из безопасного пластика"
  },
  {
    id: 38,
    name: "Кукла интерактивная с аксессуарами",
    image: "/img/d166a943-2618-4918-b162-2f653f5ae829.jpg",
    category: "Игрушки и товары для детей",
    seller: "АО «ДетМир»",
    verified: true,
    price: 3450,
    oldPrice: 3850,
    unit: "за штуку",
    minOrder: "20 штук",
    available: "150+ штук",
    rating: 4.5,
    reviews: 167,
    inStock: true,
    discount: 10,
    description: "Интерактивная кукла с голосовыми функциями и набором одежды"
  },
  {
    id: 39,
    name: "Крем для лица увлажняющий 50мл",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Косметика и парфюмерия",
    seller: "ООО «КосметикОпт»",
    verified: true,
    price: 850,
    unit: "за тубу",
    minOrder: "100 туб",
    available: "1,000+ туб",
    rating: 4.3,
    reviews: 198,
    inStock: true,
    description: "Увлажняющий крем с натуральными экстрактами"
  },
  {
    id: 40,
    name: "Шампунь для всех типов волос 1л",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Косметика и парфюмерия",
    seller: "АО «БьютиПро»",
    verified: true,
    price: 650,
    oldPrice: 750,
    unit: "за флакон",
    minOrder: "50 флаконов",
    available: "2,000+ флаконов",
    rating: 4.4,
    reviews: 234,
    inStock: true,
    discount: 13,
    description: "Профессиональный шампунь с натуральными экстрактами"
  },
  {
    id: 41,
    name: "Дрель ударная аккумуляторная 18В",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Инструменты",
    seller: "АО «ИнструментПро»",
    verified: true,
    price: 8500,
    unit: "за штуку",
    minOrder: "5 штук",
    available: "120+ штук",
    rating: 4.7,
    reviews: 145,
    inStock: true,
    description: "Профессиональная дрель с двумя аккумуляторами"
  },
  {
    id: 42,
    name: "Набор отверток 12 предметов с магнитными наконечниками",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Инструменты",
    seller: "ТД «МастерТул»",
    verified: true,
    price: 1850,
    oldPrice: 2100,
    unit: "за набор",
    minOrder: "20 наборов",
    available: "300+ наборов",
    rating: 4.6,
    reviews: 89,
    inStock: true,
    discount: 12,
    description: "Качественный набор отверток с эргономичными ручками"
  },
  {
    id: 43,
    name: "Корм для собак сухой премиум класса 15кг",
    image: "/img/764d08a6-7946-4b1d-9c27-48e192211cc0.jpg",
    category: "Животные и зоотовары",
    seller: "ООО «ЗооОпт»",
    verified: true,
    price: 3200,
    unit: "за мешок",
    minOrder: "20 мешков",
    available: "400+ мешков",
    rating: 4.8,
    reviews: 234,
    inStock: true,
    description: "Сбалансированный корм для собак с натуральными ингредиентами"
  },
  {
    id: 44,
    name: "Наполнитель для кошачьего туалета 10л",
    image: "/img/43a9e543-590b-44fb-b286-827f5d9b1ef7.jpg",
    category: "Животные и зоотовары",
    seller: "АО «ПетПродукт»",
    verified: true,
    price: 450,
    oldPrice: 520,
    unit: "за упаковку",
    minOrder: "100 упаковок",
    available: "1,000+ упаковок",
    rating: 4.2,
    reviews: 167,
    inStock: true,
    discount: 13,
    description: "Впитывающий наполнитель без запаха с антибактериальными добавками"
  },
  {
    id: 45,
    name: "Учебник по математике 5 класс (комплект)",
    image: "/img/d166a943-2618-4918-b162-2f653f5ae829.jpg",
    category: "Книги и образование",
    seller: "ООО «УчебЛит»",
    verified: true,
    price: 650,
    unit: "за комплект",
    minOrder: "200 комплектов",
    available: "5,000+ комплектов",
    rating: 4.5,
    reviews: 98,
    inStock: true,
    description: "Комплект учебников по математике с рабочими тетрадями"
  },
  {
    id: 46,
    name: "Атлас мира географический (А4 формат)",
    image: "/img/eb347072-5079-42a8-9320-9ff8ccc544f5.jpg",
    category: "Книги и образование",
    seller: "АО «ГеоИздат»",
    verified: true,
    price: 350,
    oldPrice: 400,
    unit: "за штуку",
    minOrder: "100 штук",
    available: "2,000+ штук",
    rating: 4.4,
    reviews: 167,
    inStock: true,
    discount: 13,
    description: "Подробный географический атлас с контурными картами"
  },
  {
    id: 47,
    name: "Ноутбук 15.6\" 8ГБ/256ГБ SSD",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Компьютеры и IT",
    seller: "АО «КомпТех»",
    verified: true,
    price: 45000,
    oldPrice: 48000,
    unit: "за штуку",
    minOrder: "3 штуки",
    available: "50+ штук",
    rating: 4.7,
    reviews: 89,
    inStock: true,
    discount: 6,
    description: "Производительный ноутбук для офиса и учебы"
  },
  {
    id: 48,
    name: "Монитор 24\" Full HD IPS",
    image: "/img/764d08a6-7946-4b1d-9c27-48e192211cc0.jpg",
    category: "Компьютеры и IT",
    seller: "ТД «МониторПро»",
    verified: true,
    price: 12800,
    unit: "за штуку",
    minOrder: "10 штук",
    available: "150+ штук",
    rating: 4.5,
    reviews: 156,
    inStock: true,
    description: "Профессиональный монитор с IPS-матрицей"
  },
  {
    id: 49,
    name: "Экскаватор гусеничный JCB JS330LC",
    image: "/img/e7d14dd1-2dbd-4ef4-a9fa-16c8b4ff8b33.jpg",
    category: "Спецтехника",
    seller: "ООО «СпецТехАльянс»",
    verified: true,
    price: 8500000,
    oldPrice: 9200000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "3 штуки",
    rating: 4.9,
    reviews: 45,
    inStock: true,
    discount: 8,
    description: "Гусеничный экскаватор массой 33 тонны для тяжелых земляных работ",
    detailedDescription: "Экскаватор JCB JS330LC – это надежная и производительная машина для выполнения сложных строительных и земляных работ. Оснащен мощным двигателем JCB DieselMax мощностью 200 л.с. и современной гидравлической системой.\n\nОсновные характеристики:\n• Рабочая масса: 33 000 кг\n• Мощность двигателя: 200 л.с. (149 кВт)\n• Глубина копания: 7,41 м\n• Радиус копания: 11,78 м\n• Высота выгрузки: 7,90 м\n• Вместимость ковша: 1,54 м³\n\nПреимущества:\n• Экономичный расход топлива\n• Комфортная кабина с системой климат-контроля\n• Интеллектуальная система управления JCB LiveLink\n• Надежная гидравлика с защитой от перегрузок\n• Легкое техническое обслуживание\n\nИдеально подходит для строительства, дорожных работ, разработки карьеров и коммунального хозяйства."
  },
  {
    id: 50,
    name: "Погрузчик фронтальный CAT 950GC",
    image: "/img/e7d14dd1-2dbd-4ef4-a9fa-16c8b4ff8b33.jpg",
    category: "Спецтехника",
    seller: "ООО «КатерпиллярТрейд»",
    verified: true,
    price: 6800000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "2 штуки",
    rating: 4.8,
    reviews: 32,
    inStock: true,
    description: "Колесный фронтальный погрузчик для погрузочно-разгрузочных работ"
  },
  {
    id: 51,
    name: "Токарный станок 16К20",
    image: "/img/c75818f8-e6f1-4921-ab0a-99f7e8109e4d.jpg",
    category: "Промышленное оборудование",
    seller: "ПАО «СтанкоМаш»",
    verified: true,
    price: 1850000,
    oldPrice: 2100000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "5 штук",
    rating: 4.7,
    reviews: 28,
    inStock: true,
    discount: 12,
    description: "Универсальный токарно-винторезный станок повышенной точности",
    detailedDescription: "Токарный станок модели 16К20 предназначен для обработки различных деталей из стали, чугуна, цветных металлов и их сплавов в условиях единичного и серийного производства.\n\nТехнические характеристики:\n• Наибольший диаметр обрабатываемой заготовки: 400 мм\n• Наибольшая длина обрабатываемой заготовки: 1000 мм\n• Частота вращения шпинделя: 12,5-1600 об/мин\n• Мощность главного привода: 10 кВт\n• Класс точности: Н\n• Габариты станка: 2805×1410×1500 мм\n• Масса станка: 2800 кг\n\nВозможности обработки:\n• Точение наружных и внутренних поверхностей\n• Торцевание и подрезание уступов\n• Сверление, зенкерование, развертывание\n• Нарезание резьбы резцом и плашками\n• Нарезание метрической, дюймовой, модульной резьбы\n\nВключает трехкулачковый патрон, люнет, набор резцов."
  },
  {
    id: 52,
    name: "Фрезерный станок 6Р13",
    image: "/img/c75818f8-e6f1-4921-ab0a-99f7e8109e4d.jpg",
    category: "Промышленное оборудование",
    seller: "ООО «ПромСтанки»",
    verified: true,
    price: 2200000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "3 штуки",
    rating: 4.6,
    reviews: 19,
    inStock: true,
    description: "Универсальный горизонтально-фрезерный станок для металлообработки"
  },
  {
    id: 53,
    name: "Бетономешалка СКАУТ СБР-500",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Строительная техника",
    seller: "ТД «СтройМеханизмы»",
    verified: true,
    price: 95000,
    oldPrice: 105000,
    unit: "за штуку",
    minOrder: "2 штуки",
    available: "15 штук",
    rating: 4.5,
    reviews: 67,
    inStock: true,
    discount: 10,
    description: "Бетономешалка принудительного действия объемом 500 литров"
  },
  {
    id: 54,
    name: "Виброплита Wacker Neuson DPU 3050H",
    image: "/img/30071e99-054b-4aad-b22b-679e73394520.jpg",
    category: "Строительная техника",
    seller: "ООО «ВакерРус»",
    verified: true,
    price: 185000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "8 штук",
    rating: 4.8,
    reviews: 41,
    inStock: true,
    description: "Виброплита с двигателем Honda для уплотнения грунта и асфальта"
  },
  {
    id: 55,
    name: "Дизель-генератор SDMO J130K",
    image: "/img/de534f08-2a51-46d8-87f1-b55042f10230.jpg",
    category: "Генераторы и электростанции",
    seller: "ЗАО «ЭнергоКомплект»",
    verified: true,
    price: 1650000,
    oldPrice: 1800000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "4 штуки",
    rating: 4.9,
    reviews: 25,
    inStock: true,
    discount: 8,
    description: "Стационарная дизельная электростанция мощностью 130 кВт",
    detailedDescription: "Дизельная электростанция SDMO J130K предназначена для автономного электроснабжения промышленных объектов, строительных площадок и аварийного резервирования.\n\nТехнические характеристики:\n• Номинальная мощность: 130 кВт (163 кВА)\n• Двигатель: John Deere 4045TF270\n• Расход топлива: 23,8 л/ч при 75% нагрузки\n• Объем топливного бака: 315 л\n• Автономная работа: до 13 часов\n• Габариты: 2400×850×1450 мм\n• Масса: 1580 кг\n\nОсобенности:\n• Система автоматического запуска\n• Цифровой контроллер InteliLite AMF25\n• Синхронный генератор Leroy Somer\n• Защита от перегрузки и короткого замыкания\n• Система подогрева охлаждающей жидкости\n• Шумозащитный кожух\n\nИдеально подходит для непрерывной работы в тяжелых условиях."
  },
  {
    id: 56,
    name: "Генератор бензиновый Honda EU30is",
    image: "/img/de534f08-2a51-46d8-87f1-b55042f10230.jpg",
    category: "Генераторы и электростанции",
    seller: "ООО «ХондаРоссия»",
    verified: true,
    price: 320000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "12 штук",
    rating: 4.7,
    reviews: 89,
    inStock: true,
    description: "Инверторный генератор мощностью 3 кВт для чувствительной техники"
  },
  {
    id: 57,
    name: "Автопогрузчик электрический STILL RX20-16",
    image: "/img/43a9e543-590b-44fb-b286-827f5d9b1ef7.jpg",
    category: "Складская техника",
    seller: "ООО «СтиллЛифт»",
    verified: true,
    price: 1450000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "6 штук",
    rating: 4.6,
    reviews: 34,
    inStock: true,
    description: "Электрический погрузчик грузоподъемностью 1,6 тонны для складских работ"
  },
  {
    id: 58,
    name: "Гидравлическая тележка 2,5т",
    image: "/img/43a9e543-590b-44fb-b286-827f5d9b1ef7.jpg",
    category: "Складская техника",
    seller: "ТД «СкладТехника»",
    verified: true,
    price: 28000,
    oldPrice: 32000,
    unit: "за штуку",
    minOrder: "3 штуки",
    available: "25 штук",
    rating: 4.4,
    reviews: 112,
    inStock: true,
    discount: 13,
    description: "Гидравлическая тележка для перемещения грузов на поддонах"
  },
  {
    id: 59,
    name: "Микроскоп Carl Zeiss Axio Scope.A1",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Лабораторное оборудование",
    seller: "ООО «ЦейссОптика»",
    verified: true,
    price: 850000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "2 штуки",
    rating: 4.9,
    reviews: 15,
    inStock: true,
    description: "Профессиональный исследовательский микроскоп с оптикой Carl Zeiss"
  },
  {
    id: 60,
    name: "Аналитические весы Sartorius Secura",
    image: "/img/1896fbdf-f98d-49a3-9193-25c98958adcf.jpg",
    category: "Лабораторное оборудование",
    seller: "ЗАО «ЛабПрибор»",
    verified: true,
    price: 420000,
    oldPrice: 465000,
    unit: "за штуку",
    minOrder: "1 штука",
    available: "4 штуки",
    rating: 4.8,
    reviews: 23,
    inStock: true,
    discount: 10,
    description: "Прецизионные аналитические весы с дискретностью 0.1 мг"
  }
];